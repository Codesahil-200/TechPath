...
categoryButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        if (activeBtn) {
            activeBtn.classList.remove('active', 'bg-indigo-600', 'text-white', 'shadow-lg', 'shadow-indigo-500/20');
            activeBtn.classList.add('text-slate-400', 'hover:text-white', 'hover:bg-slate-800');
        }

        const targetBtn = e.target; // or e.currentTarget if there were children
        targetBtn.classList.add('active', 'bg-indigo-600', 'text-white', 'shadow-lg', 'shadow-indigo-500/20');
