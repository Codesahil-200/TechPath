...
// 3. USER TRACKING
// ==============================================================================
// We check which specific roles the user has already registered for.
let registeredRoles = JSON.parse(localStorage.getItem('techPath_registered_roles')) || [];
...
function openChatbot() {
    // If no specific role context, use a default general career helper context.
    const role = currentChatRole || pendingRole || {
        title: "Career Assistant",
        skills: ["Career Planning", "Resume Building", "Tech Trends"],
        salary: "$0 - ∞",
        resources: ["TechPath Guide"],
        description: "I am your general Career AI Assistant. Ask me anything about finding the right IT role for you!"
    };

    document.getElementById('chatbot-role-title').innerText = role.title;
    
    chatbotModal.classList.remove('hidden');
    chatbotModal.classList.add('flex');
    setTimeout(() => {
        chatbotContent.classList.remove('scale-95', 'opacity-0');
        chatbotContent.classList.add('scale-100', 'opacity-100');
    }, 10);

    // Initial greeting if no messages yet
    if (chatMessages.children.length <= 1) {
        // Already has one greeting in HTML, let's keep it or reset if needed
    }
}

function closeChatbot() {
    chatbotContent.classList.remove('scale-100', 'opacity-100');
...
function addMessage(text, isUser = false) {
    ...
    innerDiv.className = `p-3.5 rounded-2xl shadow-sm ${
        isUser 
        ? 'bg-indigo-600 text-white' 
        : 'bg-slate-800 text-slate-200'
    }`;
    innerDiv.style.maxWidth = '90%';
    
    if (isUser) {
        innerDiv.style.borderTopRightRadius = '4px';
    } else {
        innerDiv.style.borderTopLeftRadius = '4px';
        innerDiv.style.border = '1px solid rgba(51, 65, 85, 0.5)';
    }
    
    innerDiv.innerHTML = `<p class="leading-relaxed" style="font-size: 13px;">${text}</p>`;
...
function showTypingIndicator() {
    ...
    indicator.innerHTML = `
        <div class="bg-slate-800 p-3.5 rounded-2xl shadow-sm flex items-center justify-center" style="gap: 4px; height: 40px; width: 64px; border-top-left-radius: 4px; border: 1px solid rgba(51, 65, 85, 0.5);">
...
async function handleChatSubmit(e) {
    ...
    setTimeout(() => {
        indicator.remove();
        const response = generateAIResponse(text, currentChatRole);
        addMessage(response);
    }, 1500);
}

function generateAIResponse(query, role) {
    const q = query.toLowerCase();
    
    if (!role) return "I'm sorry, I seem to have lost track of which career we're discussing. Could you please specify?";

    if (q.includes('skill') || q.includes('learn')) {
        return `To become a ${role.title}, you'll need to master: ${role.skills.join(', ')}. Which one would you like to know more about?`;
    }
    
    if (q.includes('salary') || q.includes('earn') || q.includes('money')) {
        return `The average salary for a ${role.title} ranges from ${role.salary}. This can vary based on your location and experience level.`;
    }
    
    if (q.includes('roadmap') || q.includes('start') || q.includes('how')) {
        const steps = role.roadmap.map(s => s.step).join(' -> ');
        return `The path to becoming a ${role.title} usually follows these steps: ${steps}. You can see the full details in the roadmap section of the modal!`;
    }
    
    if (q.includes('resource') || q.includes('book') || q.includes('course')) {
        return `Some great resources for ${role.title} include: ${role.resources.join(', ')}. I highly recommend checking out ${role.resources[0]} first!`;
    }
    
    if (q.includes('hi') || q.includes('hello') || q.includes('hey')) {
        return `Hello! How can I help you today with your journey towards becoming a ${role.title}?`;
    }

    if (q.includes('thank')) {
        return "You're very welcome! Feel free to ask more questions if you have any. Good luck on your TechPath!";
    }

    // Default response using role description
    return `As a ${role.title}, you'll be ${role.description.toLowerCase()} It's a very exciting and rewarding field! Do you have specific questions about the skills needed or the salary?`;
}

// Event Listeners for Chatbot
if (openChatbotBtn) openChatbotBtn.addEventListener('click', openChatbot);
if (closeChatbotBtn) closeChatbotBtn.addEventListener('click', closeChatbot);
if (floatingChatbotBtn) floatingChatbotBtn.addEventListener('click', () => {
...
function openModal(role) {
    // Check if the user has already registered for THIS specific role
    const alreadyRegisteredForThis = registeredRoles.includes(role.id);
    ...
    currentChatRole = role; // Set current role for the chatbot
    
    // Reset Chatbot Messages for new role
    chatMessages.innerHTML = `
        <div class="flex justify-start">
            <div class="bg-slate-800 text-slate-200 p-3.5 rounded-2xl shadow-sm animate-fade-in" style="max-width: 90%; border-top-left-radius: 4px; border: 1px solid rgba(51, 65, 85, 0.5);">
                <p class="leading-relaxed" style="font-size: 13px;">Hello! I'm your Career AI Assistant. I can help you understand more about being a <span class="text-indigo-400 font-bold">${role.title}</span>. Ask me anything about skills, salary, or the daily life of this role!</p>
            </div>
        </div>
    `;
...
function closeRegistrationModal() {
    const regContent = document.getElementById('reg-modal-content');
    regContent.classList.remove('scale-100', 'opacity-100');
    regContent.classList.add('scale-95', 'opacity-0');
    setTimeout(() => {
        regModal.classList.add('hidden');
    }, 300);
}

if (closeRegModal) closeRegModal.addEventListener('click', closeRegistrationModal);
...
regForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // 2. Track that this SPECIFIC role is now registered
        if (pendingRole && !registeredRoles.includes(pendingRole.id)) {
            registeredRoles.push(pendingRole.id);
            localStorage.setItem('techPath_registered_roles', JSON.stringify(registeredRoles));
        }

        // 3. Save common user info for convenience in future forms
        localStorage.setItem('techPath_user_data', JSON.stringify(userData));
